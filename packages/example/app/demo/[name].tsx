...
function ButtonDemo() {
  return (
    <VStack space="s">
      ...
      <HStack space="s">
        <Button label="加载中..." variant="filled" isLoading onPress={() => {}} />
...
function PasswordInputDemo() {
  const [value, setValue] = useState('');
  return <PasswordInput placeholder="请输入密码" value={value} onChangeText={setValue} />;
}

function NumberInputDemo() {
  const [value, setValue] = useState(5);
  return <NumberInput value={value} onChange={setValue} min={0} max={99} step={1} />;
}
...
function TextareaDemo() {
  const [value, setValue] = useState('');
  return <Textarea placeholder="请输入内容..." value={value} onChangeText={setValue} rows={4} />;
}
...
function AccordionDemo() {
  ...
  return (
    <VStack space="s">
      ...
      <Accordion type="multiple" defaultIndex={[0]} lazyMount unmountOnCollapse isAnimated={false}>
...
function AlertDemo() {
  return (
    <VStack space="s">
      ...
      <Alert variant="warning" title="警告" message="请注意此操作" />
      ...
      <Alert variant="info" closable onClose={() => {}}>
        <HStack flex={1} alignItems="center" justifyContent="space-between" space="s">
          <HStack flex={1} alignItems="center" space="s">
            <Alert.Icon />
...
function ToastDemo() {
  ...
  return (
    <>
      ...
      <HStack space="s" flexWrap="wrap" marginTop="s">
        <Button
          label="队列触发 5 条"
          size="sm"
          onPress={() => {
...
function StepsDemo() {
  ...
  return (
    <VStack space="l">
      ...
      <Steps
        current={current}
        items={[{ title: '账号信息' }, { title: '身份验证' }, { title: '完成注册' }]}
      />
      <HStack space="s">
        <Button
          label="上一步"
      ...
      <Steps
        current={1}
        items={[{ title: '提交订单' }, { title: '支付', status: 'error' }, { title: '完成' }]}
      />
      ...
      <Steps
        current={1}
        size="sm"
        items={[{ title: '步骤一' }, { title: '步骤二' }, { title: '步骤三' }]}
      />
      <Text variant="caption" color="textSecondary">
        Medium（默认）
      </Text>
